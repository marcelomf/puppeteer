...
import { MarkdownDocumenterFeatureContext, } from '@microsoft/api-documenter/lib/plugin/MarkdownDocumenterFeature.js';
...
import { ApiClass, ApiDeclaredItem, ApiDocumentedItem, ApiInitializerMixin, ApiInterface, ApiItemKind, ApiOptionalMixin, ApiParameterListMixin, ApiPropertyItem, ApiProtectedMixin, ApiReadonlyMixin, ApiReleaseTagMixin, ApiReturnTypeMixin, ApiStaticMixin, ApiTypeAlias, ExcerptTokenKind, ReleaseTag, } from '@microsoft/api-extractor-model';
import { DocCodeSpan, DocFencedCode, DocLinkTag, DocNodeKind, DocParagraph, DocPlainText, DocSection, StandardTags, StringBuilder, DocHtmlStartTag, DocHtmlEndTag, DocHtmlAttribute, } from '@microsoft/tsdoc';
...
export class MarkdownDocumenter {
    ...
    _writeApiItemPage(apiItem) {
        ...
        if (apiItem instanceof ApiDocumentedItem) {
            ...
            if (tsdocComment) {
                decoratorBlocks.push(...tsdocComment.customBlocks.filter(block => {
                    return (block.blockTag.tagNameWithUpperCase ===
                        StandardTags.decorator.tagNameWithUpperCase);
                }));
                if (tsdocComment.deprecatedBlock) {
                    output.appendNode(new DocNoteBox({ configuration: this._tsdocConfiguration }, [
        ...
        if (apiItem instanceof ApiDeclaredItem) {
            if (apiItem.excerpt.text.length > 0) {
                output.appendNode(new DocHeading({ configuration, title: 'Signature:', level: 4 }));
                let code;
        ...
        if (decoratorBlocks.length > 0) {
            output.appendNode(new DocHeading({ configuration, title: 'Decorators:', level: 4 }));
    ...
    _writeHeritageTypes(output, apiItem) {
        ...
        if (apiItem instanceof ApiTypeAlias) {
            const refs = apiItem.excerptTokens.filter(token => {
    ...
    _writeDefaultValueSection(output, apiItem) {
        if (apiItem instanceof ApiDocumentedItem) {
    ...
    _writeRemarksSection(output, apiItem) {
        ...
        if (apiItem instanceof ApiDocumentedItem) {
            ...
            if (tsdocComment) {
                ...
                const exampleBlocks = tsdocComment.customBlocks.filter(x => {
                    return (x.blockTag.tagNameWithUpperCase ===
    ...
    _writeThrowsSection(output, apiItem) {
        if (apiItem instanceof ApiDocumentedItem) {
            ...
            if (tsdocComment) {
                ...
                const throwsBlocks = tsdocComment.customBlocks.filter(x => {
                    return (x.blockTag.tagNameWithUpperCase ===
    ...
    _createTitleCell(apiItem, plain = false) {
        const configuration = this._tsdocConfiguration;
    ...
    _createDescriptionCell(apiItem) {
        ...
        if (apiItem instanceof ApiDocumentedItem) {
            const isExperimental = apiItem.tsdocComment?.modifierTagSet.isExperimental();
            if (isExperimental) {
                section.appendNodesInParagraph([
                    new DocEmphasisSpan({ configuration, bold: true, italic: true }, [
        ...
        if (apiItem instanceof ApiDocumentedItem) {
            if (apiItem.tsdocComment !== undefined) {
                ...
                if (apiItem.tsdocComment.deprecatedBlock) {
                    ...
                    section.appendNodes(apiItem.tsdocComment.deprecatedBlock.content.getChildNodes());
                }
            }
        }
        if (apiItem instanceof ApiDocumentedItem) {
    ...
    _getSidebarLabelForApiItem(apiItem) {
    ...
    _getFilenameForApiItem(apiItem, link = false) {
        if (apiItem.kind === ApiItemKind.Package) {
            return 'index.md';
        }
        let baseName = '';
            ...
            if (link) {
                switch (hierarchyItem.kind) {
                    ...
                    case ApiItemKind.PropertySignature:
                        suffix =
                            '#' +
                                Utilities.getSafeFilenameForName(PackageName.getUnscopedName(hierarchyItem.displayName));
                        break;
                }
            }
        }
...
//# sourceMappingURL=custom_markdown_documenter.js.map