...
describe('Coverage specs', function () {
    ...
    describe('JSCoverage', function () {
        it('should work', async () => {
            ...
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].url).toContain('/jscoverage/simple.html');
            (0, expect_1.default)(coverage[0].ranges).toEqual([
                { start: 0, end: 17 },
                { start: 35, end: 61 },
            ]);
        });
        it('should report sourceURLs', async () => {
            ...
            const coverage = await page.coverage.stopJSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].url).toBe('nicename.js');
        });
        ...
        it('should not ignore eval() scripts if reportAnonymousScripts is true', async () => {
            ...
            (0, expect_1.default)(filtered).toHaveLength(1);
        });
        it('should ignore pptr internal scripts if reportAnonymousScripts is true', async () => {
            ...
            const coverage = await page.coverage.stopJSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(0);
        });
        it('should report multiple scripts', async () => {
            ...
            const coverage = await page.coverage.stopJSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(2);
            coverage.sort((a, b) => {
                return a.url.localeCompare(b.url);
            });
            (0, expect_1.default)(coverage[0].url).toContain('/jscoverage/script1.js');
            (0, expect_1.default)(coverage[1].url).toContain('/jscoverage/script2.js');
        });
        it('should report right ranges', async () => {
            ...
            (0, expect_1.default)(entry.ranges).toHaveLength(2);
            const range1 = entry.ranges[0];
            (0, expect_1.default)(entry.text.substring(range1.start, range1.end)).toBe('\n');
            const range2 = entry.ranges[1];
            (0, expect_1.default)(entry.text.substring(range2.start, range2.end)).toBe(`console.log('used!');if(true===false)`);
        });
        it('should report right ranges for "per function" scope', async () => {
            ...
            (0, expect_1.default)(entry.text.substring(range1.start, range1.end)).toBe('\n');
            const range2 = entry.ranges[1];
            (0, expect_1.default)(entry.text.substring(range2.start, range2.end)).toBe(`console.log('used!');if(true===false)console.log('unused!');`);
        });
        it('should report scripts that have no coverage', async () => {
            ...
            (0, expect_1.default)(coverage).toHaveLength(1);
            const entry = coverage[0];
            (0, expect_1.default)(entry.url).toContain('unused.html');
            (0, expect_1.default)(entry.ranges).toHaveLength(0);
        });
        it('should work with conditionals', async () => {
            ...
            const coverage = await page.coverage.stopJSCoverage();
            (0, expect_1.default)(JSON.stringify(coverage, null, 2).replace(/:\d{4,5}\//g, ':<PORT>/')).toBeGolden('jscoverage-involved.txt');
        });
        ...
        describe('includeRawScriptCoverage', function () {
            it('should not include rawScriptCoverage field when disabled', async () => {
                ...
                const coverage = await page.coverage.stopJSCoverage();
                (0, expect_1.default)(coverage).toHaveLength(1);
                (0, expect_1.default)(coverage[0].rawScriptCoverage).toBeUndefined();
            });
            it('should include rawScriptCoverage field when enabled', async () => {
                ...
                const coverage = await page.coverage.stopJSCoverage();
                (0, expect_1.default)(coverage).toHaveLength(1);
                (0, expect_1.default)(coverage[0].rawScriptCoverage).toBeTruthy();
            });
        });
        // @see https://crbug.com/990945
        it.skip('should not hang when there is a debugger statement', async () => {
    ...
    describe('CSSCoverage', function () {
        it('should work', async () => {
            ...
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].url).toContain('/csscoverage/simple.html');
            (0, expect_1.default)(coverage[0].ranges).toEqual([{ start: 1, end: 22 }]);
            const range = coverage[0].ranges[0];
            (0, expect_1.default)(coverage[0].text.substring(range.start, range.end)).toBe('div { color: green; }');
        });
        it('should report sourceURLs', async () => {
            const { page, server } = await (0, mocha_utils_js_1.getTestState)();
            await page.coverage.startCSSCoverage();
            ...
            const coverage = await page.coverage.stopCSSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].url).toBe('nicename.css');
        });
        it('should report multiple stylesheets', async () => {
            ...
            (0, expect_1.default)(coverage[0].url).toContain('/csscoverage/stylesheet1.css');
            (0, expect_1.default)(coverage[1].url).toContain('/csscoverage/stylesheet2.css');
        });
        it('should report stylesheets that have no coverage', async () => {
            ...
            const coverage = await page.coverage.stopCSSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].url).toBe('unused.css');
            (0, expect_1.default)(coverage[0].ranges).toHaveLength(0);
        });
        it('should work with media queries', async () => {
            ...
            const coverage = await page.coverage.stopCSSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].url).toContain('/csscoverage/media.html');
            (0, expect_1.default)(coverage[0].ranges).toEqual([
                { start: 8, end: 15 },
                { start: 17, end: 38 },
            ]);
        ...
        it('should work with complicated usecases', async () => {
            ...
            const coverage = await page.coverage.stopCSSCoverage();
            (0, expect_1.default)(JSON.stringify(coverage, null, 2).replace(/:\d{4,5}\//g, ':<PORT>/')).toBeGolden('csscoverage-involved.txt');
        });
        it('should work with empty stylesheets', async () => {
            ...
            const coverage = await page.coverage.stopCSSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(1);
            (0, expect_1.default)(coverage[0].text).toEqual('');
        });
        it('should ignore injected stylesheets', async () => {
            ...
            (0, expect_1.default)(margin).toBe('10px');
            const coverage = await page.coverage.stopCSSCoverage();
            (0, expect_1.default)(coverage).toHaveLength(0);
        });
...
//# sourceMappingURL=coverage.spec.js.map