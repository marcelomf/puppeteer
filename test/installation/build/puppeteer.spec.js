...
describe('`puppeteer`', () => {
    ...
    it('runs in the browser', async function () {
        ...
        fs.cpSync(join(EXAMPLES_DIR, 'puppeteer-in-browser'), puppeteerInBrowserPath, {
            recursive: true,
        });
        spawnSync('npm', ['ci'], {
...
(platform() === 'win32' ? describe.skip : describe)('`puppeteer` clears cache', () => {
    configureSandbox({
        dependencies: ['@puppeteer/browsers', 'puppeteer-core', 'puppeteer'],
        env: cwd => {
            return {
    ...
    it('evaluates', async function () {
        assert.equal(readdirSync(join(this.sandbox, '.cache', 'puppeteer', 'chrome')).length, 1);
        await this.runScript(await readAsset('puppeteer', 'installCanary.js'), 'mjs');
        assert.equal(readdirSync(join(this.sandbox, '.cache', 'puppeteer', 'chrome')).length, 2);
