...
import { PUPPETEER_CORE_PACKAGE_PATH, PUPPETEER_PACKAGE_PATH, PUPPETEER_BROWSERS_PACKAGE_PATH, } from './constants.js';
 ...
 */
export const configureSandbox = (options) => {
    before(async function () {
        ...
        if (devDependencies.length > 0) {
            await execFile(PKG_MANAGER, [ADD_PKG_SUBCOMMAND, '-D', ...devDependencies], {
                cwd: sandbox,
                env,
                shell: true,
            });
        ...
        this.env = env;
        this.runScript = async (content, type, args) => {
